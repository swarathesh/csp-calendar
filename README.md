...
## Growth lab: chart and ending-value ranges

Open `site/growth.html` to compare investment examples. Change starting money, monthly deposits, and the 1–40 year horizon. Hover or drag the chart to update the main balance and date; keyboard arrows, Home, End, and Escape work too. The contribution line shares the balance scale, including when deposits exceed the account value. Its checkbox hides the comparison line. Horizon buttons change the investment length, not the historical data resolution; chart points are annual.

The table below the chart uses every available consecutive period in Real periods mode (79 examples for a 20-year horizon in the 1928–2025 dataset). Mix years uses 5,000 deterministic sampled sequences, with no repeated year within a sequence. Changing the chart's single example does not redraw this distribution. Deposit changes use the same sampled sequences, making comparisons reproducible.

Ending balances are grouped using the 10th, 25th, 75th and 90th percentile cutoffs. Each row shows the actual minimum/maximum, count and share of the examples in that group. Ties stay together and empty groups are omitted, including a single 100% row for zero deposits. Displayed dollar amounts and percentages are rounded. These frequencies are not forecasts of future probabilities: historical periods overlap, shuffled paths disrupt real sequences, and future outcomes may lie outside the observed ranges. Fees, tax and inflation are excluded. The expandable methodology explains the monthly approximation from annual total returns.

Run `node --test tests/test_growth.cjs` to check contributions, sample counts, frequency coverage, tied outcomes, deterministic sampling, input validation and shared chart scaling. These tests also run in the deployment workflow.
