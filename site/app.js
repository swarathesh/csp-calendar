...
function renderEvents(){
 ...
 $("events").innerHTML=rows.map(e=>'<tr><td>'+day(e.date)+'<small>'+esc(e.time)+'</small></td><td>'+esc(e.title)+(e.symbol?'<small>'+esc(e.symbol)+'</small>':'')+'</td><td><span class="tag '+(e.kind==="Macro"?'macro':'')+'">'+esc(e.kind)+'</span></td><td><a target="_blank" rel="noopener noreferrer" href="'+url(e.source)+'">Verify ↗</a></td></tr>').join("");
...
function card(c){
 ...
 (c.missing_sources?.length?'<p>Missing calendar sources: '+esc(c.missing_sources.join(", "))+'. Timing cannot be cleared.</p>':'')+
